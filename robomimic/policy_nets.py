...
from robomimic.models.obs_nets import MIMO_MLP, RNN_MIMO_MLP, MIMO_Transformer, MIMO_Transformer_Vanilla, MIMO_Transformer_Finish, ObservationDecoder
...
class ActorNetwork(MIMO_MLP):
    ...
    def __init__(
        ...
        # self.classification_double = classification_double
...
class ActorNetwork_Gripper_Separate(MIMO_MLP):
    """
    A basic policy network that predicts actions from observations.
    Can optionally be goal conditioned on future observations.
    """
    ...
    def _get_output_shapes(self):
        ...
        """
        return OrderedDict(action=(self.ac_dim,),
                           gripper = (self.gripper_dim,))

    # def output_shape(self, input_shape=None):
    #     return [self.ac_dim]

    def forward(self, obs_dict, goal_dict=None):
        out = super(ActorNetwork, self).forward(obs=obs_dict, goal=goal_dict)
        actions = out["action"]
        grippers = out["gripper"]
        # apply tanh squashing to ensure actions are in [-1, 1]
        actions = F.relu(actions)
        
        if self.gripper_dim == 1:
            grippers = torch.tanh(actions)
        else:
            assert self.gripper_dim == 2
            grippers = F.relu(actions)
        print(torch.cat((actions, grippers), dim=1).shape)
        exit()
        return torch.cat((actions, grippers), dim=1)
...
class GaussianActorNetwork(ActorNetwork):
    ...
    def forward_train(self, obs_dict, goal_dict=None, return_params = False):
        """
...
class GMMActorNetwork(ActorNetwork):
    ...
    def forward_train(self, obs_dict, goal_dict=None, save_path = None):
        """
        ...
        scales = out["scale"]
        logits = out["logits"]
        # print(means.shape) #torch.Size([100, 5, 7])
        # print(type(means)) #<class 'torch.Tensor'>
        
        # print(scales.shape) #torch.Size([100, 5, 7])
        
        # print(logits.shape) #<class 'torch.Tensor'>
        # print(type(logits)) #torch.Size([100, 5])
        # exit()
    ...
    def forward(self, obs_dict, goal_dict=None, save_path = None):
        """
        Samples actions from the policy distribution.
        ...
        Returns:
            action (torch.Tensor): batch of actions from policy distribution
        """
        dist = self.forward_train(obs_dict, goal_dict, save_path)
        

        # For a one-dimensional distribution, simply plot a histogram
        # samples = dist.sample((1000,))  # Sample 1000 points from the distribution
        # plt.hist(samples.cpu().numpy(), bins=50, density=True)
        # plt.title('Samples from GMM Distribution')
        # plt.xlabel('Value')
        # plt.ylabel('Density')
        # plt.show()
    ...
    def forward_for_residual(self, obs_dict, goal_dict=None):
        out = MIMO_MLP.forward(self, obs=obs_dict, goal=goal_dict)
        means = out["mean"]
        scales = out["scale"]
        logits = out["logits"]
...
class GMMResidualNetwork(ActorNetwork):
    """
    Variant of actor network that learns a multimodal Gaussian mixture distribution
    over actions.
    """
    def __init__(
        ...
        super(GMMResidualNetwork, self).__init__(
            obs_shapes=obs_shapes,
            ac_dim=ac_dim,
            mlp_layer_dims=mlp_layer_dims,
            goal_shapes=goal_shapes,
    ...
    def forward_train_with_base(self, obs_dict, gmm_means_weight, gmm_scales_weight, gmm_logits_weight, goal_dict=None):
        """
        Return full GMM distribution, which is useful for computing
        quantities necessary at train-time, like log-likelihood, KL 
        divergence, etc.
        ...
        means = obs_dict["base_means"].view(means.shape) + gmm_means_weight * means
        scales = obs_dict["base_scales"].view(scales.shape) + gmm_scales_weight * scales
        logits = obs_dict["base_logits"].view(logits.shape) + gmm_logits_weight * logits
...
class TransformerActorNetwork_Vanilla(MIMO_Transformer_Vanilla):
    """
    ...
    def __init__(
        ...
        output_shapes = self._get_output_shapes()
        super(TransformerActorNetwork_Vanilla, self).__init__(
            input_obs_group_shapes=observation_group_shapes,
            output_shapes=output_shapes,
    ...
    def output_shape(self, input_shape):
        ...
        T = input_shape[mod][0]
        TensorUtils.assert_size_at_dim(input_shape, size=T, dim=0, 
                msg="TransformerActorNetwork_Vanilla: input_shape inconsistent in temporal dimension")
        return [T, self.ac_dim]
    ...
    def forward(self, obs_dict, actions=None, goal_dict=None):
        ...
        outputs = super(TransformerActorNetwork_Vanilla, self).forward(**forward_kwargs)
...
class TransformerActorNetwork(MIMO_Transformer):
    ...
    def forward(self, obs_dict, actions=None, goal_dict=None):
        ...
        # apply tanh squashing to ensure actions are in [-1, 1]
        outputs["action"] = torch.tanh(outputs["action"])
        outputs["mlp_action"] = torch.tanh(outputs["mlp_action"])

        return outputs["action"], outputs["mlp_action"] # only action sequences
...
class TransformerFinishNetwork(MIMO_Transformer_Finish):
    """
    An Transformer policy network that predicts actions from observation sequences (assumed to be frame stacked
    from previous observations) and possible from previous actions as well (in an autoregressive manner).
    """
    def __init__(
        ...
        output_shapes = self._get_output_shapes()
        super(TransformerFinishNetwork, self).__init__(
            mlp_layer_dims = mlp_layer_dims,
            finish_layer_dims = finish_layer_dims,
            end2end = end2end,
            input_obs_group_shapes=observation_group_shapes,
            output_shapes=output_shapes,
            transformer_embed_dim=transformer_embed_dim,
    ...
    def forward(self, obs_dict, actions=None, goal_dict=None):
        ...
        # exit()
        forward_kwargs = dict(obs=obs_dict, goal=goal_dict)
        outputs = super(TransformerFinishNetwork, self).forward(**forward_kwargs)

        # apply tanh squashing to ensure actions are in [-1, 1]
        outputs["action"] = torch.tanh(outputs["action"])
        outputs["mlp_action"] = torch.tanh(outputs["mlp_action"])

        return outputs["finish_outputs"], outputs["action"], outputs["mlp_action"] # only action sequences
...
class TransformerGMMActorNetwork(TransformerActorNetwork):
    ...
    def forward_train(self, obs_dict, actions=None, goal_dict=None, low_noise_eval=None):
        ...
        means = outputs["mean"]
        # print(means.shape) #torch.Size([100, 10, 5, 7]) (B, T, num_modes, action_dim)
        # exit()
        # print(self.use_tanh) #False
        scales = outputs["scale"]
        logits = outputs["logits"]
