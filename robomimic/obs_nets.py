...
class ExtendedLinear(nn.Linear):
    ...
    def output_shape(self):
        # For a Linear layer, the output shape is just the out_features
...
class ObservationEncoder(Module):
    ...
    def __init__(self, feature_activation=nn.ReLU):
        ...
        # print(self.obs_shapes) #OrderedDict()
        # exit()
    ...
    def _create_layers(self):
        ...
        if "base_classifications" in self.obs_shapes:
            # print(self.obs_shapes["base_classifications"])
            # exit()
            self.obs_nets["base_classifications"] = ExtendedLinear(self.obs_shapes["base_classifications"], int(np.sqrt(self.obs_shapes["base_classifications"]))+1)

        if "base_actions" in self.obs_shapes:
            # print(self.obs_shapes["base_classifications"])
            # exit()
            self.obs_nets["base_actions"] = ExtendedLinear(self.obs_shapes["base_actions"], int(np.sqrt(self.obs_shapes["base_actions"]))+1)
    ...
    def forward(self, obs_dict):
        ...
        for k in self.obs_shapes:
            # print(k)           
            x = obs_dict[k]
            # print("x_init shape:{}".format(x.shape)) #object: torch.Size([100, 14])
            # maybe process encoder input with randomizer
            if self.obs_randomizers[k] is not None:
                print("obs_randomnizer of {} is not None".format(k))
                print(self.obs_randomizers[k])
                exit()
                x = self.obs_randomizers[k].forward_in(x)
            # maybe process with obs net
            if self.obs_nets[k] is not None:
                # print("obs_net of {} is not None".format(k))
                # print(self.obs_nets[k])
            ...
            # print("x_final shape:{}".format(x.shape)) #object: torch.Size([100, 14])
            # print(k)
            # print(x.shape)
            feats.append(x)
    ...
    def output_shape(self, input_shape=None):
        ...
        for k in self.obs_shapes:
            ...
            if self.obs_nets[k] is not None:
                ...
                feat_shape = self.obs_nets[k].output_shape()
                # print(feat_shape)
            if self.obs_randomizers[k] is not None:
...
class MIMO_Transformer_Vanilla(Module):
    """
    ...
    def __init__(
        ...
        """
        super(MIMO_Transformer_Vanilla, self).__init__()
    ...
    def forward(self, **inputs):       
        for obs_group in self.input_obs_group_shapes:
            for k in self.input_obs_group_shapes[obs_group]:
                # if k == 'base_classifications':
                #     self.input_obs_group_shapes[obs_group][k] = [12]
                # if k== 'base_actions':
                #     self.input_obs_group_shapes[obs_group][k] = [6]

                # print(k) #object, robot0_eef_pos, robot0_eef_quat, robot0_gripper_qpos
                # first two dimensions should be [B, T] for inputs
                # print(self.input_obs_group_shapes[obs_group][k])
                # exit()
                if inputs[obs_group][k] is None:
                    continue
                if k!= 'base_classifications' and k!='user_actions':
                    assert inputs[obs_group][k].ndim - 2 == len(self.input_obs_group_shapes[obs_group][k])
                # if not inputs[obs_group][k].ndim - 2 == len(self.input_obs_group_shapes[obs_group][k]):
                #     print(k) #base_classifications
                #     print(inputs[obs_group][k].ndim) #2
                #     print(self.input_obs_group_shapes[obs_group][k]) #[12]
                #     exit()
        ...
        assert transformer_outputs.ndim == 3  # [B, T, D]
...
class MIMO_Transformer(Module):
    ...
    def __init__(
        ...
        self.nets["user_encoder"] = nn.Linear(
            3, transformer_embed_dim
        )
        ...
        activation=nn.ReLU
        self.nets["mlp"] = MLP(
            input_dim=transformer_embed_dim*2,
            output_dim=mlp_layer_dims[-1],
            layer_dims=mlp_layer_dims[:-1],
            layer_func=layer_func,
            activation=activation,
        ...
        self.nets["decoder_w_user"] = ObservationDecoder(
            decode_shapes=self.output_shapes,
            input_feat_dim=mlp_layer_dims[-1],
        )
    ...
    def forward(self, **inputs):       
        for obs_group in self.input_obs_group_shapes:
            for k in self.input_obs_group_shapes[obs_group]:
                ...
                if inputs[obs_group][k] is None:
                    continue
                if k!= 'base_classifications' and k!='user_actions':
                    assert inputs[obs_group][k].ndim - 2 == len(self.input_obs_group_shapes[obs_group][k]), (obs_group,k,inputs[obs_group][k].ndim,self.input_obs_group_shapes[obs_group][k])
                # if not inputs[obs_group][k].ndim - 2 == len(self.input_obs_group_shapes[obs_group][k]):
                #     print(k) #base_classifications
                #     print(inputs[obs_group][k].ndim) #2
                #     print(self.input_obs_group_shapes[obs_group][k]) #[12]
        ...
        user_inputs = inputs['obs']['user_actions'].clone()
        user_embedding = self.nets["user_encoder"](user_inputs)
        inputs_wo_user = copy.deepcopy(inputs)
        del inputs_wo_user['obs']['user_actions']

        transformer_encoder_outputs = None
        transformer_inputs = TensorUtils.time_distributed(
            inputs_wo_user, self.nets["encoder"], inputs_as_kwargs=True
        )
        assert transformer_inputs.ndim == 3  # [B, T, D]
        ...
        transformer_outputs = transformer_encoder_outputs
        # print(transformer_outputs.shape) #(100,10,512)
        # exit()
        assert transformer_outputs.ndim == 3  # [B, T, D]
        # print(transformer_encoder_outputs.shape)
        # print(user_embedding.shape)
        if self.end2end:
            mlp_inputs = torch.cat((transformer_encoder_outputs[:,-1,:], user_embedding), dim=1)
        else:
            mlp_inputs = torch.cat((transformer_encoder_outputs[:,-1,:].detach().clone(), user_embedding), dim=1)
        # apply decoder to each timestep of sequence to get a dictionary of outputs
        transformer_outputs = TensorUtils.time_distributed(
            transformer_outputs, self.nets["decoder"]
        )
        
        mlp_outputs = self.nets["mlp"](mlp_inputs)
        mlp_outputs = self.nets["decoder_w_user"](mlp_outputs)
        transformer_outputs["mlp_action"] = mlp_outputs['action']
...
class MIMO_Transformer_Finish(Module):
    """
    Extension to Transformer (based on GPT architecture) to accept multiple observation 
    dictionaries as input and to output dictionaries of tensors. Inputs are specified as 
    a dictionary of observation dictionaries, with each key corresponding to an observation group.
    ...
    def __init__(
        ...
        Args:
            ...
            encoder_kwargs (dict): observation encoder config
        """
        super(MIMO_Transformer_Finish, self).__init__()
    ...
    def forward(self, **inputs):       
        ...
        finish_inputs = transformer_inputs.clone()[:,-1,:]
        finish_outputs = self.nets["finish_mlp"](finish_inputs)
        ...
        transformer_outputs["transformer_encoder_outputs"] = transformer_encoder_outputs
        transformer_outputs["finish_outputs"] = finish_outputs
        return transformer_outputs
